...
def clean_and_map_nse_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mapping = {cols[strike_pos]: "Strike"}
    for i, c in enumerate(cols):
        c_up = str(c).upper()
        if i < strike_pos:
            if "CHNG" in c_up or "CHG" in c_up:
                mapping[c] = "Call_Chng_OI"
            elif "OPEN INTEREST" in c_up or c_up == "OI":
                mapping[c] = "Call_OI"
            elif "VOLUME" in c_up:
...
def evaluate_quadruple_verified_buyer_engine(
    ...
    sub_df = sub_df[(sub_df[iv_col] >= 10.0) & (sub_df[iv_col] <= 65.0)]
    if sub_df.empty:
        return pd.DataFrame()

    sub_df["Strike_Distance"] = abs(sub_df["Strike"] - spot_price)
    sub_df = sub_df[sub_df["Strike_Distance"] <= (spot_price * 0.02)]
    if sub_df.empty:
        return pd.DataFrame()

    median_vol = sub_df[vol_col].median() if not sub_df[vol_col].empty else 1.0
    active_vol_mask = sub_df[vol_col] >= max(500.0, median_vol * 0.8)
...
# Main UI Workflow Execution
uploaded_file = st.file_uploader("📂 Upload Official NSE Option Chain CSV Export", type=["csv"])

if uploaded_file is not None:
    try:
        raw_df = load_and_parse_nse_csv(uploaded_file.read())
        ...
        if df.empty:
            ...
            if is_no_trade_zone:
                ...
                ce_df = df[(df["Call_LTP"] >= 35.0) & (df["Call_LTP"] <= 200.0)]
                pe_df = df[(df["Put_LTP"] >= 35.0) & (df["Put_LTP"] <= 200.0)]
                ...
                if combined_ranked.empty or combined_ranked["Buyer_Score"].max() <= 0:
                    ...
                    if ltp_price <= 0:
                        ...
                        # Advanced Mathematical Levels for Naked Buyers
                        limit_buy_price = round(ltp_price * 0.975, 2)  # Optimal pullback limit price on minor dip
                        stop_loss = round(ltp_price * 0.72, 2)         # Strict 28% max risk drawdown
                        risk_amount = ltp_price - stop_loss
                        
                        target_1 = round(ltp_price + (risk_amount * 1.6), 2)  # First target (Book 50% qty)
                        target_2 = round(ltp_price + (risk_amount * 2.8), 2)  # Second extended target (Trailing run)
    ...
    st.info("📁 Upload your official NSE Option Chain CSV file to initiate the quadruple-verified terminal.")
